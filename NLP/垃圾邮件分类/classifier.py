...
def get_data():
    ...
    '''
    with open("data/ham.txt", encoding="utf8") as ham_f, open("data/spa.txt", encoding="utf8") as spam_f:
        ham_data = ham_f.readlines()
        spam_data = spam_f.readlines()
        ham_label = np.ones(len(ham_data)).tolist()#[1.0,1.0,1.0,1.0]
        spam_label = np.zeros(len(spam_data)).tolist()#[0,0,0,0]
...
def main():
    ...
    corpus, labels = remove_empty_docs(corpus, labels)#去换行符，空格
    # 对数据进行划分
    ...
    bow_test_features = bow_vectorizer.transform(norm_test_corpus)#将测试文本转化为特征向量
